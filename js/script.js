const users = [
    ...
    {
        "name" : "nina johnson",
        "image" : "https://randomuser.me/api/portraits/thumb/women/68.jpg",
        "joined" : "05/09/05"
    },
    {
        "name" : "ellen takala",
...
var $contactPerPage = 10; //number of contacts per page
var $pagesNeeded = Math.ceil($total / $contactPerPage);
...
function changePageHeader() {
    ...
    content.innerHTML = `<h2>Contacts</h2><h3>Total: ${$total}</h3>`;
}

function showPage($pageNum){
    var $i = ($pageNum-1) * $contactPerPage;
    var $to = $pageNum * $contactPerPage;
    const contactList = document.querySelector(`.contact-list`);

    var $list = "";
    for ($i; $i<$to && $i<$total; $i++){
        $list += '<li class="contact-item cf">' + '\n';
        $list += '   <div class="contact-details">' + '\n';
        $list += '      <img class="avatar" src=' + users[$i].image + '>' + '\n';
        $list += '      <h3>' + users[$i].name + '</h3>' + '\n';
        $list += '      <span class="email">' + users[$i].name.replace(' ','.') + '@example.com</span>' + '\n';
        $list += '   </div>' + '\n';
        $list += '   <div class="joined-details">' + '\n';
        $list += '      <span class="date">Joined ' + users[$i].joined + '</span>' + '\n';
        $list += '   </div>' + '\n';
        $list += '</li>' + '\n';
    }
